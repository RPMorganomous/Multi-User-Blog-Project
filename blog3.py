...
class DeletePost(BlogHandler):
    def get(self, post_id):
        ...
        comment_delete_query = post.comments_query  #Comment.query().filter(Comment.post == self.key)  
        print Comment.query().filter(Comment.post == post.key).order(-Comment.last_touch_date_time)
        print "more comments"
        print comment_delete_query
...
#insert query or filter to find and delete all comments with str(comment_obj.post.id()) = post_id
        #comment_query = Comment.query(Comment.post.id() == post_id)
        for each_comment in post.comments_query:
            each_comment.key.delete()
            
        #post.comments_query.delete()
...
class DeleteComment(BlogHandler):
    ...
    def post(self, comment_id):
        key = ndb.Key('Comment', int(comment_id)) # old way but without parent key
        comment_obj = key.get()
